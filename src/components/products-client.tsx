...
import type { Product, ProductCandidate } from "@/types";
import {
  PackagePlus,
  CheckCircle2,
  XCircle,
...
export function ProductsClient({ products, candidates: initialCandidates }: ProductsClientProps) {
  ...
  const handleApprove = async (id: string) => {
    try {
      const res = await fetch(`/api/candidates/${id}`, {
        ...
        body: JSON.stringify({ status: "approved", reviewNote: "承認済み" }),
      });
      if (res.ok) {
        setCandidates((prev) =>
          prev.map((c) =>
  ...
  const handleReject = async (id: string) => {
    try {
      const res = await fetch(`/api/candidates/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ status: "rejected", reviewNote: "却下" }),
      });
      if (res.ok) {
        setCandidates((prev) =>
          prev.map((c) =>
